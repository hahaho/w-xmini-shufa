...
Component({
  ...
  methods: {
    _choosePay: function _choosePay(e) {
      this.setData({
    ...
    _closeT: function _closeT() {
      this._close(0);
    },
    _close: function _close(integral) {
      this.setData({
        show: false,
        userInputValue: null,
        currentIndex: -1
    ...
    _getUserInfo: function _getUserInfo(newValue) {
      var that = this;
      app.wxrequest({
        url: app.getUrl().info,
        data: {
    ...
    _showMask: function _showMask(newValue, oldValue, changePath) {
